...
import { FormattedResponse, speechText } from "./utils/responseFormatting";
...
export default function App() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setAnswer(result); speak(speechText(result.response));
  ...
  return <main className="app-shell">
    ...
    <form onSubmit={submit} aria-label="Legal question form">
      ...
      <textarea id="question" value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(event) => {
        if (event.key === "Enter" && !event.shiftKey) {
          event.preventDefault();
          event.currentTarget.form?.requestSubmit();
        }
      }} placeholder="For example: The police will not register my FIR." rows={5} />
      <div className="actions">
        ...
        <button type="button" className="secondary-button" onClick={stopSpeaking}>Stop speaking</button>
      </div>
    ...
    {answer && <section className="answer" aria-live="polite">
      ...
      <h2>Response</h2><FormattedResponse text={answer.response} />
